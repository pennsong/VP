...
//			mConnectionTimer = new Timer(1000 * 60 * 30);
 		...
 		override protected function doLookup(identify:String):void
 		{
 			if (mHttpService)
 			{
 				var request:Object = new Object();
				request.taskType = 'callfriend';
				request.friendIdentity = identify;
		...
		{	
			...
			if (result.hasOwnProperty("result"))
			{
				...
				else if (result.result.hasOwnProperty("callFriend"))
				{
					// party query response
					remote = result.result.callFriend as Object;
					if (remote.hasOwnProperty("userName") && remote.hasOwnProperty("identity"))
					{
						var identityString:String = remote.identity
						var userString:String = remote.userName;
